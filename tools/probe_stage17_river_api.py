...
def response_summary(result: dict[str, Any]) -> dict[str, Any]:
    ...
    text, encoding = (
        decode(body, result["headers"].get("content-type"))
        if body
        else ("", None)
    )
...
def main() -> None:
    ...
    for target in STATIC_TARGETS:
        ...
        static_reports.append({
            "id": target["id"],
            **{key: value for key, value in summary.items() if key != "parsed"},
        })
    ...
    for station in STATIONS:
            ...
            for itmknd_cd in station["itmkndCdCandidates"]:
                for obs_cd in station["obsCdCandidates"]:
                    api_urls = [
                        (
                            "api_current",
                            f"{ORIGIN}/api/tmObsStage/{station['ofcCd']}/{itmknd_cd}/"
                            f"{obs_cd}/{encoded_time}/true",
                        ),
                        (
                            "api_past_datetime",
                            f"{ORIGIN}/api/tmObsStage/past/{station['ofcCd']}/{itmknd_cd}/"
                            f"{obs_cd}/{encoded_time}",
                        ),
                        (
                            "api_past_date",
                            f"{ORIGIN}/api/tmObsStage/past/{station['ofcCd']}/{itmknd_cd}/"
                            f"{obs_cd}/{encoded_date}",
                        ),
                    ]
        ...
        static_attempts = [item for item in attempts if item["mode"].startswith("static_")]
        ...
        station_reports.append({
            ...
            "appTime": app_time,
            "attempts": attempts,
            "masterMetadataAvailable": (
                master_attempt["status"] == 200 and master_attempt["jsonParsed"]
            ),
            "masterIdentityMatched": (
                master_attempt["expectedNameFound"] and master_attempt["riverNameFound"]
            ),
            "staticSeriesSuccessCount": sum(
                1 for item in static_attempts
                if item["status"] == 200 and item["jsonParsed"]
            ),
            "apiSuccessCount": sum(
                1 for item in api_attempts
                if item["status"] == 200 and item["jsonParsed"]
            ),
        })
    ...
    missing_master_metadata = [
        station["id"] for station in station_reports
        if not station["masterMetadataAvailable"]
    ]
    master_identity_mismatches = [
        station["id"] for station in station_reports
        if not station["masterIdentityMatched"]
    ]
    static_series_station_count = sum(
        1 for station in station_reports if station["staticSeriesSuccessCount"] > 0
    )
    api_station_count = sum(
        1 for station in station_reports if station["apiSuccessCount"] > 0
    )
    ...
    report = {
        ...
        "routeBasis": {
            ...
            "apiBase": "/api",
            "apiCurrentTemplate": (
                "/tmObsStage/{ofcCd}/{itmkndCd}/{obsCd}/{appTime}/{isCurrent}"
            ),
