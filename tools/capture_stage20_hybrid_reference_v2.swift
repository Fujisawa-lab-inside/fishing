...
@MainActor
final class HybridCaptureController: NSObject, WKNavigationDelegate {
    ...
    init(application: NSApplication, pageURL: URL, screenshotURL: URL, resultURL: URL) {
        ...
        self.webView = WKWebView(frame: NSRect(x: 0, y: 0, width: 1000, height: 860), configuration: configuration)
    ...
    private func poll() {
        ...
        (() => {
          ...
          return {
            ...
            meshVersion: document.querySelector('#mesh-version').textContent,
            snapshots: document.querySelector('#snapshots').textContent,
            cells: document.querySelector('#cells').textContent,
            pack: document.querySelector('#pack').textContent,
            output: document.querySelector('#output').textContent,
            synthesis: document.querySelector('#synthesis').textContent,
...
guard arguments.count == 4,
    ...
    FileHandle.standardError.write(Data("usage: capture_stage20_hybrid_reference_v2.swift PAGE_URL SCREENSHOT RESULT_JSON\n".utf8))
    exit(2)
}
