    ...
    private func readInterface() {
        ...
        webView.evaluateJavaScript(script) { [weak self] value, error in
            ...
            guard record["phase"] as? String == "ready",
                  ...
                  (record["mapGateLabel"] as? String)?.contains("提供座標") == true,
                  record["gateFieldsUnchanged"] as? Bool == true,
                  record["gatePositionSource"] as? String == "provided-geojson",
                  (record["canvasAria"] as? String)?.contains("番号位置は提供緯度経度") == true,
