#!/usr/bin/env python3
"""Run the approved bounded H2 dynamics comparison on R20 and multi-zone meshes.

This diagnostic is intentionally local and disposable.  It does not replace a
production mesh, change the public solver, run precomputation, or select a
physical fishway discharge.  The current approved topology is implemented
directly: gates 1-8 are binary internal interfaces, fixed barrage parts are
walls, and the approved P2 fishway transfer is always enabled.
"""
...
def _advance_h2_step(
    ...
    for face in range(len(boundary_cells)):
        ...
        if tag == 0:
            ...
            ghost_normal_momentum = 2.0 * target_flux_by_tag[tag] - normal_momentum
