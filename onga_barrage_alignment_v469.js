...
(function(){
  ...
  function noStand(a,b){ return !!(window.ONGA_SPATIAL_SAFETY && window.ONGA_SPATIAL_SAFETY.noStandAt && window.ONGA_SPATIAL_SAFETY.noStandAt(a,b)); }
  ...
  const green = rawGreen.map(q=>{ const p=rawToAligned(q[0],q[1]); return [p.lat,p.lng]; }).filter(q=>Number.isFinite(q[0])&&Number.isFinite(q[1])&&!noStand(q[0],q[1]));
  function nearestAlignedGreen(target){ if(!target||!Number.isFinite(target.lat)||!Number.isFinite(target.lng))return null; const max=(typeof CAST_MODEL!=='undefined'&&CAST_MODEL.maxM)||100; let best=null; for(const p of green){ const d=hav(p[0],p[1],target.lat,target.lng); if(d<2||d>max||castBlocked(p[0],p[1],target.lat,target.lng))continue; const score=(target.score||0)*(.7+.3*Math.exp(-.5*((d-35)/32)**2)); if(!best||score>best.score)best={lat:p[0],lng:p[1],distanceM:d,score,landConfidence:1,bankQuality:1,onBarrageAlignedBoundary:true}; } return best; }
