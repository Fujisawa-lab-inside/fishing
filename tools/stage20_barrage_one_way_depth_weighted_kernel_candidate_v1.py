#!/usr/bin/env python3
"""Review-only H2 kernel candidate with conservative one-way barrage faces.

The numerical body is copied from the bound depth-weighted candidate.  Normal
faces and outward barrage faces retain that path.  An adverse oriented barrage
face uses the already-computed reflected-wall terms on both sides, so no mass
crosses toward the bound upstream cell.  This module is not connected to
production, duration runs, precompute, GUI, or public paths.
"""
...
class OneWayBarrageStepWithTraceV1(NamedTuple):
    """One numerical step plus limiter and blocked-reverse-face evidence."""
    ...
    fishway_source_residual_m3_s: float
    limiter_sample: Any
    blocked_reverse_face_count: int
    blocked_reverse_potential_discharge_m3_s: float
...
def _advance_h2_step_depth_weighted_boundary_with_trace_core_v1(
    ...
    cfl_target: float,
    maximum_dt: float,
    one_way_upstream_cell_by_face: np.ndarray,
) -> tuple[Any, ...]:
    cell_count = len(areas)
    residual = np.zeros((cell_count, 3), dtype=np.float64)
    denominator = np.zeros(cell_count, dtype=np.float64)
    blocked_reverse_face_count = 0
    blocked_reverse_potential_discharge = 0.0

    if len(one_way_upstream_cell_by_face) != len(left_cells):
        raise ValueError("one-way upstream orientation length mismatch")
    ...
    for face in range(len(left_cells)):
        ...
        upstream_cell = one_way_upstream_cell_by_face[face]
        if upstream_cell < -1 or (
            upstream_cell >= 0
            and upstream_cell != left
            and upstream_cell != right
        ):
            raise ValueError("invalid one-way upstream cell orientation")
        one_way_face = upstream_cell >= 0
        if multiplier < 1.0 or one_way_face:
            ...
            if one_way_face:
                raw_outward_mass_flux = (
                    flux_0 if upstream_cell == left else -flux_0
                )
                if multiplier <= 0.0:
                    left_term_0 = wall_l0
                    left_term_1 = wall_l1
                    left_term_2 = wall_l2
                    right_term_0 = wall_r0
                    right_term_1 = wall_r1
                    right_term_2 = wall_r2
                elif raw_outward_mass_flux < 0.0:
                    blocked_reverse_face_count += 1
                    blocked_reverse_potential_discharge += (
                        -raw_outward_mass_flux * multiplier * length
                    )
                    left_term_0 = wall_l0
                    left_term_1 = wall_l1
                    left_term_2 = wall_l2
...
def advance_h2_step_depth_weighted_boundary(
    ...
    result = _advance_h2_step_depth_weighted_boundary_with_trace_core_v1(
        ...
        cfl_target,
        maximum_dt,
        np.full(len(left_cells), -1, dtype=np.int64),
    )
...
def advance_h2_step_barrage_one_way_with_trace_v1(
    state: np.ndarray,
    bed: np.ndarray,
    manning: np.ndarray,
    areas: np.ndarray,
    ...
    maximum_dt: float,
    one_way_upstream_cell_by_face: np.ndarray,
) -> OneWayBarrageStepWithTraceV1:
    """Advance once with explicit oriented one-way barrage-face branches."""
    ...
    orientation = np.asarray(one_way_upstream_cell_by_face, dtype=np.int64)
    if orientation.shape != np.asarray(left_cells).shape:
        raise ValueError("one-way upstream orientation shape mismatch")
    result = _advance_h2_step_depth_weighted_boundary_with_trace_core_v1(
        state,
        bed,
        manning,
    ...
    maximum_dt_bound = float(result[15])
    selected_bound_index = int(result[16])
    blocked_count = int(result[17])
    blocked_potential = float(result[18])
    if not (
        coverage_complete
        and expected_cell_count > 0
        and evaluated_cell_count == expected_cell_count
        ...
        and selected_bound_index in (0, 1)
        and blocked_count >= 0
        and math.isfinite(blocked_potential)
        and blocked_potential >= 0.0
    ):
        raise ValueError("incomplete or nonfinite one-way limiter trace")
    selected_bound_name = (
        CFL_DT_BOUND_NAME
        if selected_bound_index == 0
        else MAXIMUM_DT_BOUND_NAME
    ...
    return OneWayBarrageStepWithTraceV1(
        next_state=result[0],
        accepted_dt_s=float(result[1]),
        maximum_cfl=float(result[2]),
        boundary_outflow_m3_s=float(result[3]),
        ...
        fishway_source_residual_m3_s=float(result[5]),
        limiter_sample=limiter_sample,
        blocked_reverse_face_count=blocked_count,
        blocked_reverse_potential_discharge_m3_s=blocked_potential,
    )
