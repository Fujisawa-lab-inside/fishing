...
const cases = [
  ...
  {
    name: 'pc-stage13',
    path: '/OngaEstuarySimulator_Browser_Service_v4_6_PCFull_ConfluenceTracer.html?stage13=1',
    size: '1440,1000',
    stage13: true,
  },
  {
  ...
  {
    name: 'mobile-stage13',
    path: '/OngaEstuarySimulator_Browser_Service_v4_6_MobileLite_ConfluenceTracer.html?stage13=1',
    size: '390,844',
    stage13: true,
  },
];
...
function validateDom(testCase, dom) {
  ...
  const required = [
    ...
    'data-onga-stage13-fluid-exact-centres="true"',
    'data-onga-stage13-fluid-authority-cache-primed="true"',
...
try {
  ...
  for (const testCase of cases) {
    const result = runChrome(testCase);
    await fs.writeFile(`${outputDirectory}/${testCase.name}.html`, result.dom, 'utf8');
    await fs.writeFile(`${outputDirectory}/${testCase.name}.stderr.log`, result.stderr, 'utf8');
    validateDom(testCase, result.dom);
    summary.push({ name: testCase.name, url: result.url, ok: true, bytes: result.dom.length });
  }
  await fs.writeFile(
