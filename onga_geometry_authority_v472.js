// v4.7.2 authoritative coordinate-geometry water and hotspot patch
(function(){
  'use strict';
  if(window.__ONGA_GEOMETRY_AUTHORITY_V472__) return;
  window.__ONGA_GEOMETRY_AUTHORITY_V472__ = true;
  const VERSION='onga-geometry-authority-v4.7.2';
  ...
  function install(){
    ...
    function targetAllowed(p){return !!p&&Number.isFinite(p.lat)&&Number.isFinite(p.lng)&&explicitState(p.lat,p.lng)===true;}
    ...
    if(typeof nearestHydroCorridor==='function'){
      const prevHydro=nearestHydroCorridor;
      nearestHydroCorridor=function(lat,lng){if(water(lat,lng)<=0)return null;return (api.hydro&&api.hydro(lat,lng))||prevHydro(lat,lng);};
    }
